...
function registerSIUnit(units, { name, abbreviation, category }) {
  units.set(...unit(name, abbreviation, category));
  for (const [prefix, abbreviationPrefix] of SI_PREFIXES) {
    units.set(...unit(`${prefix}${name}`, `${abbreviationPrefix}${abbreviation}`, category));
  }
}
...
]) registerSIUnit(DEFAULT_UNITS, { name, abbreviation, category });
