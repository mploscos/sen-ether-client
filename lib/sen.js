...
export class Sen extends EventEmitter {
  ...
  async publishObjects(busName, objects, options = {}) {
    ...
    const published = await this.#runPublishedOperation(bus, async () => {
      ...
      return this.client.publishObjects(bus, descriptors, types ? { ...options, types } : options);
    });
    for (let index = 0; index < descriptors.length; index += 1) {
      this.#rememberPublishedObject(bus, { ...descriptors[index], id: published[index].id }, options);
    }
    return published;
  }
  ...
  async publish(busName, object, options = {}) {
    ...
    return this.published.get(publishedObjectKey(bus, published)).handle;
  }
