...
const RUNTIME_EVENT_DEDUPE_LIMIT = 2048;
...
async function waitForMatchingEvent(emitter, event, predicate, timeoutMs) {
...
function mergedTypeDefinitions(base, override) {
...
function collectReferencedTypeNames(typeNames, typeRegistry) {
...
function publishedObjectKey(busName, object) {
  const id = object?.id ?? crc32(String(object?.name ?? ''));
  return `${busName}\u0000${id >>> 0}`;
}

function publishedDescriptor(object) {
...
function recoverablePublicationError(error) {
...
export class SenPublishedObject {
  ...
  /**
   * Emit a SEN runtime event declared by this object's ClassTypeSpec.
   *
   * @param {string} name
   * @param {unknown[]} [args]
...
function runtimeEventDedupeKey(ownerId, item) {
...
export class Sen extends EventEmitter {
  ...
  async publishObjects(busName, objects, options = {}) {
    ...
    const published = await this.#runPublishedOperation(bus, async () => {
      await this.#ensurePublishedBus(bus, options);
      const types = mergedTypeDefinitions(this.options.types, options.types);
      return this.client.publishObjects(bus, list, types ? { ...options, types } : options);
    });
    for (const object of list) {
      this.#rememberPublishedObject(bus, object, options);
    }
    return published;
  }
  ...
  async publish(busName, object, options = {}) {
    ...
    const sessionName = this.target.session?.name ?? this.client.processInfo.sessionName;
    const bus = etherBusName(sessionName, busName);
    await this.publishObjects(bus, object, options);
    return this.published.get(publishedObjectKey(bus, object)).handle;
  }
  ...
  async updatePublishedObject(busName, object, patch, options = {}) {
    ...
    const sessionName = this.target.session?.name ?? this.client.processInfo.sessionName;
    const bus = etherBusName(sessionName, busName);
    const record = this.#publishedObject(bus, object);
    const updated = await this.#runPublishedOperation(bus, async () => {
      await this.#ensurePublishedBus(bus, options);
      return this.client.updatePublishedObject(bus, object, patch);
    });
    if (record) {
  ...
  async emitPublishedEvent(busName, object, eventName, args = [], options = {}) {
    ...
    return await this.#runPublishedOperation(bus, async () => {
      await this.#ensurePublishedBus(bus, options);
      return await this.client.emitPublishedEvent(bus, record.id, eventName, args, options);
    });
  }
  ...
  async removePublishedObjects(busName, objects, options = {}) {
    ...
    const records = selectors.map(selector => this.#publishedObject(bus, selector)).filter(Boolean);
    for (const record of records) {
      this.published.delete(record.key);
    }
    try {
      await this.#waitForReconnect();
      return this.client.removePublishedObjects(bus, objects);
    } catch (error) {
      if (recoverablePublicationError(error)) {
  ...
  async #getOrJoinBus(bus, options, timeoutMs) {
    ...
    if (!pending) {
      pending = (async () => {
        ...
        const ready = waitForMatchingEvent(
          this.client,
          'busParticipantReady',
          value => value?.busName === bus,
          participantReadyTimeoutMs
        );
        let joined;
        try {
        ...
        await ready.catch(error => this.emit('warning', error));
  ...
  #wireClient(client) {
    ...
    client.on('close', hadError => {
      // A locally hosted session remains healthy after its last inbound peer
      // leaves. EtherClient uses "close" for that per-connection state too.
      if (this.target?.local && !hadError) {
        return;
      }
      this.#stopPresenceWatchdog();
  ...
  async #reconnect() {
    ...
    for (let attempt = 1; unlimited || attempt <= maxAttempts; attempt += 1) {
      ...
      try {
        ...
        await this.#republishPublishedObjects();
  ...
  #rememberPublishedObject(bus, object, options) {
    const descriptor = publishedDescriptor(object);
    const key = publishedObjectKey(bus, descriptor);
    ...
    const record = {
      key,
      bus,
      id: (descriptor.id ?? crc32(descriptor.name)) >>> 0,
      descriptor,
      options: { ...options },
      handle: undefined
    };
    record.handle = new SenPublishedObject(this, bus, record);
    this.published.set(key, record);
    return record;
  }

  #publishedObject(bus, selector) {
    ...
    const id = typeof selector === 'number'
      ...
      : typeof selector === 'object'
        ? (selector.id ?? crc32(String(selector.name ?? ''))) >>> 0
        : undefined;
    if (id !== undefined) {
      return this.published.get(`${bus}\u0000${id}`);
    }
    const name = String(selector ?? '');
    return [...this.published.values()].find(record => record.bus === bus && record.descriptor.name === name);
  }
  ...
  async #runPublishedOperation(bus, operation) {
    try {
      await this.#waitForReconnect();
      return await operation();
    } catch (error) {
      if (!recoverablePublicationError(error) || this.manualClose) {
        throw error;
      }
      await this.#waitForReconnect();
      await this.#restorePublishedBus(bus);
      return await operation();
    }
  }

  async #ensurePublishedBus(bus, options = {}) {
  ...
  async #restorePublishedBus(bus) {
    this.buses.delete(bus);
    await this.#ensurePublishedBus(bus);
    for (const record of this.published.values()) {
      if (record.bus !== bus) continue;
      await this.#publishRecord(record);
    }
  }

  async #publishRecord(record) {
    await this.#ensurePublishedBus(record.bus, record.options);
    const types = mergedTypeDefinitions(this.options.types, record.options.types);
    return this.client.publishObjects(
      record.bus,
      record.descriptor,
  ...
  async #republishPublishedObjects() {
    for (const record of this.published.values()) {
      await this.#publishRecord(record);
    }
  }
...
export class SenBus extends EventEmitter {
  ...
  async waitForReferencedTypes(typeNames, options = {}) {
    ...
    const requestMissingTypes = missing => {
      const hashes = new Set();
      for (const typeName of missing) {
        const hash = crc32(typeName) >>> 0;
        if (this.requestedTypeHashes.has(hash)) continue;
        this.requestedTypeHashes.add(hash);
        hashes.add(hash);
      }
      if (hashes.size) this.sen.client.requestTypes(this.name, hashes);
    };

    let missing = missingTypeNames();
    if (!missing.length) return;
    requestMissingTypes(missing);
...
export class SenRemoteObject extends EventEmitter {
  ...
  applyState(buffer, source, timestamp, options = {}) {
    ...
    for (const value of values) {
      ...
      const previousTimestamp = this.propertyTimestamps.get(value.name);
      if (
