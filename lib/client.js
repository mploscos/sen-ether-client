 ...
 */
  ...
  #onBusFrame(payload, connection = undefined) {
    ...
    // A process can be the only common connection between two remote bus
    // participants (owner -> publisher and owner -> consumer). Ether control
    // frames are otherwise point-to-point, so forward the original bus frame
    // to the other process connections. Keeping `frame.to` intact preserves
    // the actual source participant id used by SEN for replies and ownership.
    if (busState && connection) {
      this.#relayBusFrame(busState, connection, payload);
    }
  ...
  #relayBusFrame(busState, incomingConnection, payload) {
