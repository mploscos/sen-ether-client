...
import { decodeArguments, encodePropertyUpdateBuffer, encodeValue } from './values.js';
import {
 ...
 */
  ...
  #onMulticastBusDatagram(busState, message, remote) {
    try {
      ...
      this.emit(busMessage.categoryName, { ...frame, busId: busState.busId, ...busMessage, remote, multicast: true });
    } catch (error) {
      this.emit('error', error);
    }
  }
