 ...
 */
  ...
  async #onRuntimeMethodCall(busState, call, connection) {
    ...
    try {
      const args = decodeArguments(call.argumentsBuffer, method.args ?? [], registry);
      const context = {
        busName: busState.busName,
        busId: busState.busId,
        object: localObject,
      ...
      const returnValue = !method.returnType || method.returnType === 'void'
        ? Buffer.alloc(0)
        : encodeValue(result, method.returnType, registry);
      this.#sendRuntimeMethodResponse(busState, connection, {
        result: 'success',
        objectId: localObject.id,
        ticketId: call.ticketId,
